...
class GooglePayResourceDefinitions {
	/******************************
     *
     ...
     *
     *******************************/
	public static function makeEventTicketClassResource($classId, array $ticketData = []) {
	    // Define the resource representation of the Class
	    // values should be from your DB/services; here we hardcode information
		...
		$localEventNameTranslated->setLanguage( get_locale());
		$localEventNameTranslated->setValue(__('Aquatonic Course', 'tmsm-aquatonic-course-booking'));
		...
		if ( ! empty( $geo_lat ) && ! empty( $geo_lng ) ) {
			...
			$location->setLatitude( $geo_lat );
			$location->setLongitude( $geo_lng );
		...
		$localVenueNameTranslated->setLanguage( get_locale());
		$localVenueNameTranslated->setValue($shop_name);
		...
		$localVenueAddressTranslated->setLanguage( get_locale());
		$localVenueAddressTranslated->setValue($address);
		...
		if( ! empty($phone_number)){
			...
			$telephoneUri->setUri('tel:' . $phone_number);
			$telephoneUri->setDescription($phone_name);
			$linksModuleData = new Google_Service_Walletobjects_LinksModuleData();
			$linksModuleData->setUris($telephoneUri);
		}
        ...
        $payload->setId($classId);
        $payload->setIssuerName($shop_name);
        $payload->setReviewStatus('underReview'); // deprecated? @link https://developers.google.com/pay/passes/rest/v1/ReviewStatus
     ...
     *
     *******************************/
	public static function makeEventTicketObjectResource($classId, $objectId, array $ticketData = []) {
	    // Define the resource representation of the Object
	    // values should be from your DB/services; here we hardcode information
		...
		$barcodenumber = $ticketData['barcode'];
		...
		$barcode->setAlternateText($barcode->getValue());
		...
		$localSeatValueTranslated->setLanguage( get_locale());
		...
		$localRowValueTranslated->setLanguage( get_locale());
		...
		$localSectionValueTranslated->setLanguage( get_locale());
		...
		$localGateValueTranslated->setLanguage( get_locale() );
		...
		*/
		...
		$payload->setTicketHolderName($ticketData['firstname'] . ' ' . $ticketData['lastname'] );
		$payload->setTicketNumber($barcodenumber);
